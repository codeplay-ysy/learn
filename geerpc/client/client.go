...
type Client struct {
	...
	mu       sync.Mutex
	seq      uint64
	pending  map[uint64]*Call //结构体传值会进行复制
	closing  bool
	shutdown bool
}
...
type newClientFunc func(conn net.Conn, otp *Option) (client *Client, err error)
...
func (client *Client) Go(serviceMethod string, args, reply interface{}, done chan *Call) *Call {
	if done == nil {
		...
		log.Panic("rpc client: done chnnel is unbuffered")
	}
	call := &Call{
...
func NewHTTPClient(conn net.Conn, opt *Option) (*Client, error) {
	_, _ = io.WriteString(conn, fmt.Sprintf("CONNET %s HTTP/1.0\n\n", defaultRPCPath))
	resp, err := http.ReadResponse(bufio.NewReader(conn), &http.Request{Method: "CONNET"})
